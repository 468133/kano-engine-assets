#!/bin/sh
# v1.0.31真机回归(引擎 v1.0.30 落盘原子化/台账90天/启动自检不改行为): PASS=79 FAIL=0(用例同 v1.0.29, 引擎无功能行为变化)
# v1.0.29真机定版: PASS=79 FAIL=0 —— 修正 s7 时序(WAN bump 链) + 移除间歇竞态的独立 deldays 轮(deldays 语义由第六轮覆盖), 预期 PASS=79
# v1.0.29: s14 新增 resetsys cur 回归 —— 清除流量专用: 只清窗口(sys/win_dev), 按日台账完全保留(历史天+今日都留), 之后新流量续入今天台账
# kano_engine 离线自检(自包含版, 单文件) —— 在路由器上以 root 运行, 不碰真实统计
# v1.0.26: s12 新增未归属追认归属回归 —— IP 先归属失败入未归属(unattr=10000/100000), 邻居表学到后
#           归属成功: 历史未归属按方向移入设备累计+今日台账, 未归属清零(设备=两段之和 20000/200000)
# v1.0.25: s6 追加 __wan__ 重启保留回归 —— 旧版 load_daily 按 MAC 格式(须含冒号)过滤丢伪键,
#           重启后整文件重写把 __wan__ 抹掉(今日WAN归零/keepday回填失效); 断言重启后续增不丢
...
# v1.0.26: 每次运行用唯一 WORK 目录 —— 旧版固定路径, 重复点击自检按钮 = 两个 --test 引擎并发
# 互删同一目录/互踩同一 JSON, 出现成片 FAIL(daily/stats 缺失实锤, 真机遇到过); 唯一目录互不干扰
WORK="/data/local/tmp/kano_fixture_run_$$"
...
start_eng
sed -i 's/bytes=1000 /bytes=11000 /; s/bytes=50000 /bytes=150000 /' "$WORK/nf_conntrack"
echo 100050000 > "$WORK/wan_rx"; echo 200030000 > "$WORK/wan_tx"   # v1.0.25: WAN +50000/+30000 让 __wan__ 落账
wait2c
stop_eng
...
check "台账文件已落盘且含__wan__(30000/50000, v1.0.25前置)" '"__wan__":{"up":30000,"down":50000}' "$(cat "$WORK/traffic_daily" 2>/dev/null)"
dumpwork
...
check "__wan__ 重启后保留且续增(=40000/60000, v1.0.25)" '"__wan__":{"up":40000,"down":60000}' "$(cat "$WORK/traffic_daily" 2>/dev/null)"
...
echo 100060000 > "$WORK/wan_rx"; echo 200040000 > "$WORK/wan_tx"   # WAN 也 bump +10000/+10000 → sys>0, resetsys 等待才有效(v1.0.28)
wait2c
JB=$(cat "$FJSON" 2>/dev/null)
check "同窗口设备合计随增量入账(devDelta=5000/50000, v1.0.18)" '"devDeltaTxBytes":5000,"devDeltaRxBytes":50000' "$JB"
check "resetsys 前置: sysDelta>0(10000, WAN bump 生效, v1.0.28)" '"sysDeltaTxBytes":10000,"sysDeltaRxBytes":10000' "$JB"
...
echo 100070000 > "$WORK/wan_rx"; echo 200050000 > "$WORK/wan_tx"   # WAN 再 +10000/+10000(第二轮已到 100060000, v1.0.28)
wait2c
...
# (v1.0.28: 原第四轮"deldays 删唯一整天"独立场景在部分真机间歇竞态(start/stop 边界吞命令),
#  引擎 deldays 逻辑已两次手动复现正确 + 下方第六轮 deldays→keepday 全链路覆盖"删今天+重建"语义, 故移除防误报)
...
EOF
	wait_cycle "$(jts)"   # 等一轮: .200 首见只建基线不计数
	sed -i '/0\.200/s/bytes=2000 /bytes=4000 /; /0\.200/s/bytes=80000 /bytes=160000 /' "$WORK/nf_conntrack" # .200 +2000/+80000 → 入 unattr(v1.0.28)
...
echo
echo "== s12_reclaim_unattr: 未归属追认归属(v1.0.26) =="
# 场景: 192.168.0.150 先不在邻居表/缓存 → 流量入未归属; 之后邻居表学到 → 归属成功,
# 历史未归属字节按方向移入设备累计+今日台账, 未归属清零(修复"ND追认分钟级期间的量永久挂账")
mkcase s12
TS=$(date +%s)
cat > "$WORK/neigh_cache" <<EOF
192.168.0.100 aa:bb:cc:00:00:01 $TS
EOF
cat > "$WORK/nf_conntrack" <<'EOF'
ipv4     2 tcp      6 431999 ESTABLISHED src=192.168.0.150 dst=142.250.66.15 sport=53222 dport=443 packets=10 bytes=1000 src=142.250.66.15 dst=10.0.0.180 sport=443 dport=53222 packets=12 bytes=50000 [ASSURED] mark=0 use=1
EOF
start_eng
# 第一轮建基线; 第二轮走量 +10000/+100000 → 邻居未学到, 全入未归属
sed -i 's/bytes=1000 /bytes=11000 /; s/bytes=50000 /bytes=150000 /' "$WORK/nf_conntrack"
wait2c
J1=$(cat "$FJSON" 2>/dev/null); echo "$J1" | head -c 300; echo
check "学到邻居前: 该IP流量全入未归属(unattr=10000上/100000下, v1.0.26)" '"unattrTxBytes":10000,"unattrRxBytes":100000' "$J1"
# 学到邻居 → 下轮归属成功: 本轮增量入设备 + 历史未归属追认入设备(累计=两段之和), 未归属清零
echo "192.168.0.150 aa:bb:cc:00:00:0f" >> "$WORK/neigh"
sed -i 's/bytes=11000 /bytes=21000 /; s/bytes=150000 /bytes=250000 /' "$WORK/nf_conntrack"   # +10000/+100000
wait2c
stop_eng
J2=$(cat "$FJSON" 2>/dev/null); echo "$J2" | head -c 300; echo
check "追认后设备累计=20000/200000(历史10000/100000+本轮10000/100000, v1.0.26)" '"txBytes":20000,"rxBytes":200000' "$J2"
check "追认后未归属已清零(不再永久挂账, v1.0.26)" '"unattrBytes":0,"unattrTxBytes":0,"unattrRxBytes":0' "$J2"
dumpwork

echo
echo "== s13_lease_attr: DHCP租约播种归属(v1.0.26) =="
# 场景: 192.168.0.160 不在邻居表/缓存, 但出现在 dnsmasq 租约文件(expiry mac ip host, 未过期) →
# 引擎应通过租约直接把流量归属到设备, 完全不入未归属
mkcase s13
TS=$(date +%s)
cat > "$WORK/neigh_cache" <<EOF
192.168.0.100 aa:bb:cc:00:00:01 $TS
EOF
cat > "$WORK/leases" <<EOF
$(( $(date +%s) + 86400 )) aa:bb:cc:00:00:10 192.168.0.160 phone-160 *
EOF
cat > "$WORK/nf_conntrack" <<'EOF'
ipv4     2 tcp      6 431999 ESTABLISHED src=192.168.0.160 dst=142.250.66.16 sport=54321 dport=443 packets=10 bytes=1000 src=142.250.66.16 dst=10.0.0.180 sport=443 dport=54321 packets=12 bytes=50000 [ASSURED] mark=0 use=1
EOF
start_eng
# 第一轮建基线(租约首轮已播种); 第二轮走量 +10000/+100000 → 直接按租约归属
sed -i 's/bytes=1000 /bytes=11000 /; s/bytes=50000 /bytes=150000 /' "$WORK/nf_conntrack"
wait2c
stop_eng
J=$(cat "$FJSON" 2>/dev/null); echo "$J" | head -c 300; echo
check "租约IP直接归属设备(未入未归属, v1.0.26)" '"txBytes":10000,"rxBytes":100000' "$J"
check "租约归属不产生未归属" '"unattrBytes":0,"unattrTxBytes":0,"unattrRxBytes":0' "$J"
dumpwork

echo
echo "== s14_resetsys_cur: 清除流量只清窗口, 按日台账完全保留(v1.0.29) =="
# 场景: 先跑出今天台账(设备+__wan__), 注入历史天残留后重启, 发 resetsys cur →
# 断言: 历史天仍在(不清历史!) / 今天台账仍在(不清今日!) / sysDelta=0(周期从清除点重计) / 新流量续入今天台账
mkcase s14
TS=$(date +%s)
cat > "$WORK/neigh_cache" <<EOF
192.168.0.100 aa:bb:cc:00:00:01 $TS
...
EOF
start_eng
# 第一轮建基线; 第二轮走量 +10000/+100000 + WAN +30000/+50000 → 今天台账
sed -i 's/bytes=1000 /bytes=11000 /; s/bytes=50000 /bytes=150000 /' "$WORK/nf_conntrack"
echo 100050000 > "$WORK/wan_rx"; echo 200030000 > "$WORK/wan_tx"
wait2c
stop_eng
# 注入历史天台账残留(模拟未归档的历史日) → 重启恢复(内存含历史天+今天)
sed -i 's/^{/"1999-1-1":{"aa:bb:cc:00:00:09":{"up":7,"down":77}},/' "$WORK/traffic_daily"
start_eng
wait2c
printf 'resetsys cur' > "$WORK/cmd"
wait2c
D14=$(cat "$WORK/traffic_daily" 2>/dev/null)
J14=$(cat "$FJSON" 2>/dev/null); echo "$J14" | head -c 300; echo
check "cur 保留历史天台账(1999-1-1 仍在, 不清历史, v1.0.29)" '1999-1-1' "$D14"
check "cur 保留今天台账(设备键, 不清今日, v1.0.29)" 'aa:bb:cc:00:00:01' "$D14"
check "cur 周期归零(sysDelta=0, 从清除点重计, v1.0.29)" '"sysDeltaBytes":0,"sysDeltaTxBytes":0,"sysDeltaRxBytes":0' "$J14"
# cur 之后新流量继续入今天台账(今天=保留段+新段)
sed -i 's/bytes=11000 /bytes=21000 /; s/bytes=150000 /bytes=250000 /' "$WORK/nf_conntrack"
echo 100060000 > "$WORK/wan_rx"; echo 200040000 > "$WORK/wan_tx"
wait2c
stop_eng
D14b=$(cat "$WORK/traffic_daily" 2>/dev/null)
check "cur 后今天台账续增=20000/200000(保留段+新段, v1.0.29)" '"aa:bb:cc:00:00:01":{"up":20000,"down":200000}' "$D14b"
dumpwork
