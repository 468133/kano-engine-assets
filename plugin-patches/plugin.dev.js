...
(async () => {
    ...
    const PLUGIN_VERSION = '22.0.0-dev';
    ...
    const resetHistoryEntriesByDate = (dateStr) => {
        // v22.0.0: 按日期重计 — 只清空目标日期之前产生的流量统计
        ...
        for (const mac of Object.keys(trafficHistory)) {
            const prev = trafficHistory[mac];
            // 如果设备首次出现时间在目标日期之后，不清空
            if ((prev.firstSeen || now) >= targetTs) continue;
            const ed = eng && eng.devices ? eng.devices[mac] : null;
            // 引擎模式下重新对齐基线
            const baseUp = ed ? (ed.txBytes || 0) : (prev.engLastUp || 0);
            const baseDown = ed ? (ed.rxBytes || 0) : (prev.engLastDown || 0);
            trafficHistory[mac] = {
                totalUp: 0, totalDown: 0, curUp: 0, curDown: 0,
                lastUp: 0, lastDown: 0,
                firstSeen: targetTs, // 重设首次出现时间为目标日期
                lastSeen: now,
                hostname: prev.hostname,
                ip: prev.ip,
                ip6s: prev.ip6s || (prev.ip6 ? [prev.ip6] : []),
        ...
        _log('ACTION', `按日期重计: ${dateStr} 清空了 ${cleared} 台设备的旧统计`);
        return cleared;
    };
    ...
    const archiveAndReset = async (label, resetCounters = true, targetDateStr = null) => {
        if (pluginUninstalled) return;
        if (_refreshing || _tickRunning) { createToast('正在同步数据，请稍后再试', 'pink', 3000); return; }
        _refreshing = true;
        try {
            await updateStats();
            const now = Date.now();
            const snap = { id: now, label: label || '归档 ' + fmtDateTime(now), time: now, reset: !!resetCounters, by: clientName, devices: {}, sysDelta: null };
            
            // v22.0.0: 系统增量归档 — 将引擎summary中的WAN口增量纳入快照
            const eng = await readEngineJson(true);
            if (eng && eng.summary) {
                const s = eng.summary;
                snap.sysDelta = {
                    up: s.sysDeltaTxBytes || 0,
                    down: s.sysDeltaRxBytes || 0,
                    total: s.sysDeltaBytes || 0,
                    iptTotal: s.iptTotalBytes || 0,
                    deviceTotal: s.deviceTotalBytes || 0,
                    unattr: s.unattrBytes || 0,
                    ctLocal: s.ctLocalBytes || 0,
            ...
            await saveSnapshots();
            _log('ACTION', `归档 ${snap.label} reset=${resetCounters} date=${targetDateStr || 'all'} 设备数=${Object.keys(snap.devices).length} 总量=${formatBytes(Object.values(snap.devices).reduce((s, d) => s + d.total, 0))}`);
            ...
            if (resetCounters) {
                ...
                if (!engOn) {
                    ...
                    // v22.0.0: 引擎模式下发送 reset 命令，让引擎同步归零计数器
                    try {
                        await _shUser(`printf '%s' 'reset' > ${ENGINE_CMD}`);
                        _log('ENGINE', '已发送 reset 命令到引擎');
                        await new Promise(r => setTimeout(r, 800));
                        _engineJsonCache = { t: 0, data: null };
                    } catch (e) { _log('ERR', `发送引擎reset命令失败: ${e&&e.message||e}`); }
                }
                
                // v22.0.0: 支持按日期重计
                if (targetDateStr) {
                    resetHistoryEntriesByDate(targetDateStr);
                } else {
                ...
                <div style="background:rgba(255,255,255,0.03);border-radius:8px;padding:10px;">
                    <div style="display:flex;align-items:center;gap:8px;">
                        ...
                        <div style="flex:1;cursor:pointer;" onclick="this.parentElement.parentElement.querySelector('.snap-detail-inline').style.display=this.parentElement.parentElement.querySelector('.snap-detail-inline').style.display==='none'?'block':'none';">
                            ...
                            <div style="opacity:.5;font-size:10px;margin-top:2px;">${fmtDateTime(snap.time)} &#xB7; ${devCount} 个设备 &#xB7; 总 ${formatBytes(totalAll)}${snap.by ? ' &#xB7; 由 ' + escHtml(snap.by) + ' 上传' : ''}${snap.sysDelta && snap.sysDelta.total > 0 ? ' &#xB7; WAN ' + formatBytes(snap.sysDelta.total) + (snap.sysDelta.unattr > 0 ? ' (未归属' + formatBytes(snap.sysDelta.unattr) + ')' : '') : ''}</div>
                        </div>
    ...
    const checkAutoArchive = async () => {
        ...
        // v22.0.0: 修复自动归档 — 增加"错过补归档"逻辑
        // 如果设备在归档时刻关机/插件未打开，开机后检测到已错过就立即补归档
        const isPastDailyTime = now.getHours() > archiveSettings.dailyHour || 
            (now.getHours() === archiveSettings.dailyHour && now.getMinutes() >= archiveSettings.dailyMinute);
        const dailyDue = archiveSettings.dailyEnabled && lastAutoArchiveDay !== dayKey && isPastDailyTime;
        
        const isPastMonthlyTime = now.getDate() > archiveSettings.monthlyDay || 
            (now.getDate() === archiveSettings.monthlyDay && now.getHours() >= archiveSettings.monthlyHour);
        const monthlyDue = archiveSettings.monthlyEnabled && lastAutoArchiveMonth !== monthKey && 
            now.getDate() >= archiveSettings.monthlyDay && isPastMonthlyTime;
    ...
    // 归档并重计 (从history移到traffic) — v22.0.0: 增加日期选择器，支持按日期归档和重计
    document.querySelector('#kano_traffic_archive_btn').addEventListener('click', () => {
        if (pluginUninstalled) return;
        const todayStr = new Date().toISOString().split('T')[0];
        const { id, el } = createModal({
            name: 'traffic_archive_modal', title: '▣ 选择归档方式', maxWidth: 'min(400px, 94vw)',
            showConfirm: true, confirmBtnText: '归档并重计', closeBtnText: '取消',
            onClose: () => true,
            onConfirm: async () => {
                const dateInput = el.querySelector('#kano_archive_date');
                const selectedDate = dateInput ? dateInput.value : null;
                await archiveAndReset(null, true, selectedDate);
                return true;
            },
            content: `<div style="font-size:13px;line-height:1.6;opacity:.85;">
                <div style="margin-bottom:10px;">
                    <label style="font-size:11px;opacity:.7;display:block;margin-bottom:4px;">归档日期（重计将清空该日期之前的累计流量）</label>
                    <input type="date" id="kano_archive_date" value="${todayStr}" max="${todayStr}" style="width:100%;padding:8px;border-radius:6px;border:1px solid rgba(255,255,255,0.1);background:rgba(255,255,255,0.05);color:inherit;font-size:13px;box-sizing:border-box;">
                    <div style="font-size:10px;opacity:.5;margin-top:3px;">留空或今天 = 清空全部；选择历史日期 = 只清空该日期前的统计</div>
                </div>
                <div style="background:rgba(167,139,250,0.08);padding:10px;border-radius:6px;margin-bottom:8px;">
                    ...
                    <div style="opacity:.6;font-size:11px;">保存当前流量快照，然后按选定日期清零计数器</div>
                </div>
