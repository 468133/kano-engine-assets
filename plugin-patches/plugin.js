...
(async () => {
    ...
    const renderOverview = () => {
        ...
        let html = `<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(56px,1fr));gap:6px;text-align:center;font-size:13px;">
            <div style="padding:8px 4px;background:rgba(255,255,255,0.03);border-radius:8px;"><div style="font-size:10px;opacity:.55">接入设备</div><div style="font-size:20px;font-weight:900;color:var(--dark-btn-color-active);">${deviceList.length}</div></div>
            <div style="padding:8px 4px;background:rgba(255,255,255,0.03);border-radius:8px;"><div style="font-size:10px;opacity:.55">总上行</div><div style="font-size:13px;font-weight:800">${formatBytes(up)}</div></div>
            <div style="padding:8px 4px;background:rgba(255,255,255,0.03);border-radius:8px;"><div style="font-size:10px;opacity:.55">总下行</div><div style="font-size:13px;font-weight:800">${formatBytes(down)}</div></div>
            <div style="padding:8px 4px;background:rgba(255,255,255,0.03);border-radius:8px;"><div style="font-size:10px;opacity:.55">总流量</div><div style="font-size:13px;font-weight:800;color:#a78bfa">${formatBytes(up+down)}</div></div>
        </div>`;
        ...
        if (rc) {
            ...
            html += `<div id="kano_unattr_row_missing" style="margin-top:6px;padding:8px 10px;background:rgba(255,255,255,0.03);border-radius:8px;font-size:11px;display:flex;align-items:center;gap:8px;line-height:1.4;">
                <span style="opacity:.7;">系统增量数据不可用</span>
                <span style="opacity:.5;">（引擎数据缺失或未写入）</span>
                <button id="kano_try_engine_btn" style="margin-left:auto;padding:4px 8px;border-radius:6px;background:rgba(59,130,246,0.12);border:1px solid rgba(59,130,246,0.18);color:#3b82f6;cursor:pointer;">一键自检/刷新</button>
            </div>`;
        }
        el.innerHTML = html;
        const urow = el.querySelector('#kano_unattr_row');
        if (urow) urow.addEventListener('click', () => showUnattrDetail());
        const tryBtn = el.querySelector('#kano_try_engine_btn');
        if (tryBtn) tryBtn.addEventListener('click', async (ev) => {
            tryBtn.disabled = true; tryBtn.textContent = '检测中...';
            try { await refresh(); } catch (e) { _log('ERR', `一键自检失败: ${e && e.message || e}`); createToast('自检失败', 'red', 3000); }
            tryBtn.disabled = false; tryBtn.textContent = '一键自检/刷新';
        });
    };
