...
(async () => {
    // ============================================================
    //  设备流量监控插件 v21.1.57 (限速修复: 按MAC动态解析IP+后台无条件重建, 前端关闭/上行临时地址均生效)
    //  v21.1.57 新增(修复): ① 限速「前端关闭后完全无效」—— 根因: 后台守护 kano_bg.sh 每60s只检查
    //    KANO_LIMIT 链"是否存在", 固件网络事件(接口重连/防火墙重载)删掉 FORWARD 跳转或清链内规则时
    //    自定义链本身还在 → 检查恒过 → 永不重建, 前端关闭后无人修复规则即失效。改: bg.sh 每60s
    //    无条件执行限速脚本(幂等: 挂链+flush+重建); ② 限速规则 IP 改"按MAC动态解析" —— 前端30s重建与
    //    后台60s重建都现查 /proc/net/arp + ip -6 neigh, DHCP 漂移/新地址不再依赖前端最后一次的快照;
    //    ③ 上行限速失效(真机诊断: v6 源方向 0 pkts)—— 根因: Android IPv6 隐私扩展默认启用, 上行连接用
    //    临时地址(非主全局地址), 而引擎模式设备 ip6s 只有引擎报的主地址 → v6 srcip 规则匹配不到 → 上行
    //    完全绕过。改: 引擎模式合并 ip -6 neigh 全部地址(含临时地址)进设备 ip6s, 限速/统计/诊断全覆盖;
    //    ④ 限速诊断③ 展示设备全部IPv6地址并标注临时地址, 判读指南补充说明
    //  v21.1.56 新增: ① 今日卡片双列 —— 像总上行/总下行一样同时显示「今日流量(按设备)」与
    //    「今日WAN总流量(引擎台账 __wan__, WAN口今天0点起, 含路由器自身与未归属部分)」;
    //    ② 「清除流量」连带清除今日 —— 引擎模式 resetsys 后重读已清空的台账文件, iptables 回退模式
    //    删前端今天段并写回; ③ 配合引擎v1.0.21: resetsys 同清未归属地址表(un_tab), 清除后归因四项
    //    同窗口无旧值残留(未归属TOP/独立记账不再显示清除前旧值)
    ...
    //  v21.1.54 新增: 「清除流量」联动清零引擎 WAN 系统增量 —— 旧版只清前端设备统计, 引擎 sysDelta 保持
    //    旧值继续累计, 差额归因会出现"设备合计≈0、未归属≈整个系统增量"的口径假象; 引擎模式下清除流量
    //    现在会向引擎发 resetsys(连带清空按日台账, 与"清除全部流量"语义一致), 系统增量从清除点重计
    //  v21.1.55 新增: ① 归因弹窗「未归属 TOP」反查实际设备 —— 按 IP/IPv6 匹配设备历史(trafficHistory)与
    //    在线表(deviceList), 直接显示 "→ 设备名"(未匹配标"未识别", 引擎仍在对这些地址做 ND/ARP 探测追认);
    //    ② 「清除流量」后轮询等引擎消费 resetsys 落地(sysDelta 归零)再刷新总览/归因, 不再等下轮 tick
    //    (约5s)才显示清零值 —— 总览/归因行立即反映"系统增量从清除点重计"
    //  v21.1.56 新增: ① 今日卡片双列 —— 像总上行/总下行一样同时显示「今日流量(按设备)」与
    //    「今日WAN总流量(引擎台账 __wan__, WAN口今天0点起, 含路由器自身与未归属部分)」;
    //    ② 「清除流量」连带清除今日 —— 引擎模式 resetsys 后重读已清空的台账文件, iptables 回退模式
    //    删前端今天段并写回; ③ 配合引擎v1.0.21: resetsys 同清未归属地址表(un_tab), 清除后归因四项
    ...
    // v21.1.56: 某日 WAN 总流量 —— 引擎台账 __wan__ 伪键(WAN口逐日全设备总量, 引擎≥v1.0.16),
    // 含路由器自身与未归属部分; 今天台账暂无该键(刚清除/旧引擎)时返回 0 —— 引擎从清除点重建,
    // 卡片显示 0B 而非误导性横杠
    const dailyWanTot = (dayKey) => {
        if (archivedDays.has(dayKey)) return 0;
        const dd = dailyData && dailyData[dayKey];
        if (!dd) return 0;
        const w = dd.__wan__;
        return w && typeof w === 'object' ? ((w.up || 0) + (w.down || 0)) : 0;
    };
    ...
    const applyLimits = async () => {
        ...
        // v21.1.57: 规则 IP 用 ARP/邻居表现查 —— 前端30s重建用最新地址, DHCP漂移/多地址/临时地址不漏
        const arpMap = {};
        const neigh6Map = {};
        try {
            const arpRaw = _sh(await runShellWithRoot(`awk '{print tolower($1), tolower($4)}' /proc/net/arp 2>/dev/null`));
            for (const line of arpRaw.split('\n')) {
                const [ip, mac] = line.trim().split(/\s+/);
                if (ip && mac && mac !== '00:00:00:00:00:00' && ip.indexOf('.') > 0) (arpMap[mac] = arpMap[mac] || []).push(ip);
            }
        } catch (e) {}
        try {
            const ndRaw = _sh(await runShellWithRoot(`ip -6 neigh show 2>/dev/null | awk '$4=="lladdr"{print tolower($1), tolower($5)}'`));
            for (const line of ndRaw.split('\n')) {
                const [ip6, mac] = line.trim().split(/\s+/);
                if (ip6 && mac) (neigh6Map[mac] = neigh6Map[mac] || []).push(ip6);
            }
        } catch (e) {}
        ...
        for (const mac of Object.keys(deviceLimits)) {
            ...
            const v4s = (arpMap[mac] && arpMap[mac].length) ? arpMap[mac] : (dev.ip ? [dev.ip] : []);
            for (const ip of v4s) cmds4.push(...buildLimitRules(IPT, LIMIT_CHAIN, ip, l, macKey));
            const v6s = (neigh6Map[mac] && neigh6Map[mac].length) ? neigh6Map[mac] : (dev.ip6s || []);
            for (const ip6 of v6s) cmds6.push(...buildLimitRules(IP6T, LIMIT_CHAIN6, ip6, l, macKey));
        }
        if (hasIptables) {
        ...
        // v21.1.57: 后台脚本改为自包含"按MAC动态解析IP" —— bg.sh 每60s无条件执行:
        // 挂链+flush+现查地址重建, 前端关闭/规则被清/IP漂移/临时地址都不再失效
        try {
            const S = [];
            S.push('#!/bin/sh');
            S.push('# kano_limits_apply v21.1.57: 按MAC动态解析IP重建限速(kano_bg.sh每60s执行)');
            S.push('bin4=iptables; bin6=ip6tables');
            S.push('command -v iptables-legacy >/dev/null 2>&1 && bin4=iptables-legacy');
            S.push('command -v ip6tables-legacy >/dev/null 2>&1 && bin6=ip6tables-legacy');
            S.push(`$bin4 -t filter -N ${LIMIT_CHAIN} 2>/dev/null || true`);
            S.push(`$bin4 -t filter -C FORWARD -j ${LIMIT_CHAIN} 2>/dev/null || $bin4 -t filter -I FORWARD 1 -j ${LIMIT_CHAIN}`);
            S.push(`$bin4 -t filter -F ${LIMIT_CHAIN} 2>/dev/null`);
            if (hasIp6tables && enableIPv6) {
                S.push(`$bin6 -t filter -N ${LIMIT_CHAIN6} 2>/dev/null || true`);
                S.push(`$bin6 -t filter -C FORWARD -j ${LIMIT_CHAIN6} 2>/dev/null || $bin6 -t filter -I FORWARD 1 -j ${LIMIT_CHAIN6}`);
                S.push(`$bin6 -t filter -F ${LIMIT_CHAIN6} 2>/dev/null`);
            }
            for (const mac of Object.keys(deviceLimits)) {
                const l = deviceLimits[mac];
                if (!l) continue;
                ...
                const macKey = mac.replace(/:/g, '');
                S.push(`# ${mac} up=${up} down=${down}`);
                if (limitMode === 'hashlimit') {
                    S.push(`for ip in $(awk '$4==tolower("${mac}"){print $1}' /proc/net/arp 2>/dev/null); do`);
                    if (up > 0) S.push(`  $bin4 -t filter -A ${LIMIT_CHAIN} -s $ip -m hashlimit --hashlimit-above ${up}kb/s --hashlimit-mode srcip --hashlimit-name ku_${macKey} -j DROP`);
                    if (down > 0) S.push(`  $bin4 -t filter -A ${LIMIT_CHAIN} -d $ip -m hashlimit --hashlimit-above ${down}kb/s --hashlimit-mode dstip --hashlimit-name kd_${macKey} -j DROP`);
                    S.push('done');
                    if (hasIp6tables && enableIPv6) {
                        S.push(`for ip in $(ip -6 neigh show 2>/dev/null | awk '$4=="lladdr" && tolower($5)=="${mac}"{print $1}'); do`);
                        if (up > 0) S.push(`  $bin6 -t filter -A ${LIMIT_CHAIN6} -s $ip -m hashlimit --hashlimit-above ${up}kb/s --hashlimit-mode srcip --hashlimit-name ku_${macKey} -j DROP`);
                        if (down > 0) S.push(`  $bin6 -t filter -A ${LIMIT_CHAIN6} -d $ip -m hashlimit --hashlimit-above ${down}kb/s --hashlimit-mode dstip --hashlimit-name kd_${macKey} -j DROP`);
                        S.push('done');
                    }
                } else if (limitMode === 'limit') {
                    const ppsUp = Math.max(1, Math.round(up * 1024 / 1500));
                    const ppsDn = Math.max(1, Math.round(down * 1024 / 1500));
                    S.push(`for ip in $(awk '$4==tolower("${mac}"){print $1}' /proc/net/arp 2>/dev/null); do`);
                    if (up > 0) { S.push(`  $bin4 -t filter -A ${LIMIT_CHAIN} -s $ip -m limit --limit ${ppsUp}/s --limit-burst ${2 * ppsUp} -j RETURN`); S.push(`  $bin4 -t filter -A ${LIMIT_CHAIN} -s $ip -j DROP`); }
                    if (down > 0) { S.push(`  $bin4 -t filter -A ${LIMIT_CHAIN} -d $ip -m limit --limit ${ppsDn}/s --limit-burst ${2 * ppsDn} -j RETURN`); S.push(`  $bin4 -t filter -A ${LIMIT_CHAIN} -d $ip -j DROP`); }
                    S.push('done');
                    if (hasIp6tables && enableIPv6) {
                        S.push(`for ip in $(ip -6 neigh show 2>/dev/null | awk '$4=="lladdr" && tolower($5)=="${mac}"{print $1}'); do`);
                        if (up > 0) { S.push(`  $bin6 -t filter -A ${LIMIT_CHAIN6} -s $ip -m limit --limit ${ppsUp}/s --limit-burst ${2 * ppsUp} -j RETURN`); S.push(`  $bin6 -t filter -A ${LIMIT_CHAIN6} -s $ip -j DROP`); }
                        if (down > 0) { S.push(`  $bin6 -t filter -A ${LIMIT_CHAIN6} -d $ip -m limit --limit ${ppsDn}/s --limit-burst ${2 * ppsDn} -j RETURN`); S.push(`  $bin6 -t filter -A ${LIMIT_CHAIN6} -d $ip -j DROP`); }
                        S.push('done');
                    }
                }
            }
            await _shUser(`echo '${btoa(unescape(encodeURIComponent(S.join('\n') + '\n')))}' | base64 -d > ${LIMITS_APPLY_SH} && chmod 755 ${LIMITS_APPLY_SH}`);
        } catch (e) {}
    };
    ...
    const fetchDevs = async () => {
        ...
        if (eng) {
            ...
            // v21.1.57: 引擎模式也合并 ip -6 neigh 全部地址(含Android临时地址) —— 引擎JSON只报主地址,
            // 上行连接用临时地址(隐私扩展)时限速srcip规则/统计/诊断才能覆盖; 与回退模式同款合并
            try {
                const nd = await getNdDevs();
                for (const n of nd) {
                    const ex = m.get(n.mac);
                    if (ex) {
                        if (!ex.ip6s.includes(n.ip6)) ex.ip6s.push(n.ip6);
                    } else {
                        const picked = pickDeviceName(n.mac, '', accessInfo, leaseNames);
                        m.set(n.mac, {
                            ip: null, ip6s: [n.ip6], mac: n.mac,
                            hostname: picked.name, autoName: picked.autoName, nameSource: picked.nameSource,
                            signal: wifiSig[n.mac] ?? null,
    ...
    const renderOverview = () => {
        ...
        let html = `<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(72px,1fr));gap:8px;text-align:center;">
            ...
            <div style="padding:12px 4px;background:rgba(255,255,255,0.03);border-radius:10px;" title="今日流量=引擎台账今日全部设备合计(含离线, 按设备今天0点起); 今日WAN总流量=WAN口今日总量(引擎台账 __wan__, 引擎≥v1.0.16 记录, 含路由器自身与未归属部分)"><div style="font-size:10px;opacity:.55;margin-bottom:4px;">今日流量 / 今日WAN</div><div style="display:flex;justify-content:center;align-items:baseline;gap:6px;flex-wrap:nowrap;"><span style="font-size:12px;font-weight:800;color:${dayColor(0)};white-space:nowrap;">${formatBytes(dailyTot(dayKeyOf(new Date())))}</span><span style="opacity:.3;">/</span><span style="font-size:12px;font-weight:800;color:#a78bfa;white-space:nowrap;">${formatBytes(dailyWanTot(dayKeyOf(new Date())))}</span></div><div style="font-size:8px;opacity:.4;margin-top:2px;">按设备 / WAN口今天0点起</div></div>
    ...
    // v21.1.55: 未归属地址 → 实际设备反查 —— 按 IP/IPv6 精确匹配设备历史(trafficHistory)与在线表(deviceList),
    // 归因弹窗直接显示"谁"在产生未归属流量; 匹配不到则标"未识别"(引擎仍在做 ND/ARP 探测追认)
    const findDeviceByIp = (ip) => {
        if (!ip) return null;
        const lookup = (mac, h) => {
            if (h.ip !== ip && !(Array.isArray(h.ip6s) && h.ip6s.indexOf(ip) >= 0)) return null;
            const cm = canonMac(mac);
            const name = customNames[cm] || h.hostname || defaultDeviceName(h.ip);
            return { mac: cm, name: name || '' };
        };
        for (const [mac, h] of Object.entries(trafficHistory)) { const r = lookup(mac, h); if (r) return r; }
        for (const d of deviceList) { const r = lookup(d.mac, d); if (r) return r; }
        return null;
    };
    ...
    const showUnattrDetail = () => {
        ...
        if (tops.length) {
            // v21.1.55: 反查实际设备 —— 匹配到的显示 "→ 设备名", 未匹配标"未识别"
            let matched = 0;
            top = '<div style="margin-top:10px;font-size:11px;opacity:.7;">未归属 TOP(按LAN侧地址聚合, → 为已匹配设备):</div>' +
                tops.map(t => {
                    const dev = findDeviceByIp(t.ip);
                    if (dev && dev.name) matched++;
                    const nameHtml = dev && dev.name
                        ? `<span style="opacity:.8;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:34%;">→ ${escHtml(dev.name)}</span>`
                        : '<span style="opacity:.4;">未识别</span>';
                    return `<div style="display:flex;justify-content:space-between;gap:6px;align-items:center;font-size:11px;font-family:monospace;padding:2px 0;"><span style="opacity:.8;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:40%;">${escHtml(t.ip)}</span>${nameHtml}<span style="white-space:nowrap;">${formatBytes(t.bytes || 0)}</span></div>`;
                }).join('');
            if (matched > 0) top += `<div style="margin-top:4px;font-size:10px;opacity:.5;">已匹配 ${matched}/${tops.length} 个地址到设备(IP 曾出现在设备历史/在线表); 未识别地址引擎正做 ND/ARP 探测, 学到 MAC 后自动追认。</div>`;
        }
    ...
    const deployEngine = async (manifest) => {
        ...
        if (text === null) {
            _log('ENGINE', '浏览器侧 CDN 下载失败，尝试设备侧下载回退');
            try {
                await deployEngineOnDevice(manifest);
                return; // 设备侧部署成功，直接返回
            } catch (e) {
                _log('ENGINE', `设备侧回退也失败: ${e && e.message || e}`);
                throw new Error('浏览器侧全部 CDN 节点下载失败');
            }
        }
    ...
    const clearAllTraffic = async () => {
        ...
        _refreshing = true;
        try {
            const engPre = await readEngineJson(true);
            const engOn = !!engPre;
            if (!engOn) {
                await flushChain();
                await initChain();
                ...
                ruleOwners = {};
                await syncRules();
                // v21.1.56: iptables 回退模式无引擎台账, 删前端今天段并写回 —— 今日卡片同步清除
                const tk = dayKeyOf(new Date());
                if (dailyData && typeof dailyData === 'object' && dailyData[tk]) { delete dailyData[tk]; await saveDaily(); }
            } else {
                // v21.1.54: 清除流量联动清零引擎 WAN 系统增量 —— 旧版只清前端设备统计, 引擎 sysDelta 保持
                // 旧值继续累计, 差额归因会出现"设备合计≈0、未归属≈整个系统增量"的口径假象;
                // 裸 resetsys 连带清空按日台账, 与"清除全部流量(保留名称)"语义一致(引擎<v1.0.11 自动忽略)
                const sent = await resetEngineSys(false);
                // v21.1.55: 轮询等引擎消费 resetsys(约1~6s)后回读, 总览/归因立即显示清零值
                if (sent) {
                    await waitEngineSysZeroed((engPre && engPre.summary && engPre.summary.sysDeltaBytes) || 0);
                    // v21.1.56: 引擎已清空按日台账, 重读文件 —— 今日流量/今日WAN卡片立即归零
                    // (引擎 v1.0.21 起 un_tab 也同清, 归因四项同窗口无旧值残留)
                    const d = await loadFromFile(DAILY_FILE);
                    if (d && typeof d === 'object') dailyData = d;
                }
                // sent=false(引擎<v1.0.11): 引擎无法清零, 今日台账也不动(口径诚实, 保持引擎窗口)
            }
            const now = Date.now();
            ...
            renderList();
            renderOverview();
            _log('ACTION', '清除全部流量(保留名称, 系统增量已同步清零)');
            createToast('已清除全部流量，系统增量已同步清零，自定义名称已保留', 'green');
    ...
    const resetEngineSys = async (keepToday) => {
        try {
            ...
            if (!engRevGe('1.0.11')) { _log('SYNC', '引擎 <v1.0.11 不支持 resetsys, 系统增量保持引擎启动窗口'); return false; }
    ...
    // v21.1.55: 清除流量后轮询等引擎把 resetsys 落地 —— 引擎 5s 循环消费 cmd, JSON 同步为 sysDelta≈0;
    // 判据: 当前系统增量 < 清除前值(清零已生效)或已归零; 超时兜底(新流量从 0 重计, 数值仍正确)
    const waitEngineSysZeroed = async (sysBefore, maxWaitMs = 8000) => {
        if (isMockMode()) { _engineJsonCache = { t: 0, data: null }; return; } // mock 每次造新值, 轮询无意义
        const t0 = Date.now();
        while (Date.now() - t0 < maxWaitMs) {
            await new Promise(r => setTimeout(r, 1000));
            const ej = await readEngineJson(true); // 每轮强刷
            const s = ej && ej.summary;
            if (!ej || !s) { _engineJsonCache = { t: 0, data: null }; return; }
            const cur = s.sysDeltaBytes || 0;
            if (cur <= 0 || cur < sysBefore) break; // 清零已生效
        }
        _engineJsonCache = { t: 0, data: null }; // renderOverview 兜底重读, 双保险
    };
    ...
    const installBgDaemon = async () => {
        ...
        try {
            ...
            const script = '#!/bin/sh\n' +
                ...
                '  if [ -f ' + LIMITS_APPLY_SH + ' ]; then sh ' + LIMITS_APPLY_SH + ' >/dev/null 2>&1; fi\n' + // v21.1.57: 无条件重建(脚本内按MAC动态解析IP+挂链+flush), 不再只检查链存在 —— 规则被清/跳转被删/前端关闭都自愈
                'done\n';
