...
(async () => {
    ...
    //  v21.0.5 改动(引擎全接管 + 版本号硬编码修复):
    //    U1 引擎在线(JSON可用)时完全停用 iptables 计数链: refresh/监控tick完整同步/新设备探针/
    //       清除全部/归档重计/删除设备/IPv6开关 全部跳过建链与规则增删, 只吃引擎 JSON;
    //       引擎掉线自动回退原 iptables 路径; 限速链(KANO_LIMIT)不受影响照常维护
    //    U2 版本号硬编码清理: 诊断弹窗/诊断包/底部信息/INIT日志统一引用 PLUGIN_VERSION,
    //       云更新后版本号显示不再滞后
    //  v21.0.6 改动(Dock化UI + 性能 + 后台守护):
    //    W1 顶部悬浮液态玻璃 Dock(◈流量/▣历史/◉设置) + 状态栏(◷系统时间/◴运行时间); 按钮id全部保留, 事件绑定不变
    //    W2 历史弹窗加"设备趋势"页: 任选设备按归档画纯CSS柱状图; W3 emoji图标换抽象几何符号(ICON常量, 各设备渲染一致)
    //    W4 手动"手机视图"开关删除 —— 容器级 kano-compact 自动隐藏上/下载列, 总流量列加小字行(↑↓明细+速率)兜底
    //    V1 引擎在线时 fetchDevs 不再读DHCP租约(引擎自带主机名, 每轮少1次shell); V2 WiFi信号加 iwinfo 兜底
    //    V3 限速规则持久化 kano_limits_apply.sh; V4 后台守护 kano_bg.sh(60s循环: 到点写归档触发文件/限速链被清重建),
    //       开机自启幂等写入, 页面开着时每60s由心跳主端消费触发文件执行归档, 卸载全清理
    //    V5 诊断加 TUN 代理检测(0.0.0.0/1分流或tun网卡), 引擎在线提示"统计不受影响", 否则建议装引擎
    //    V6 引擎在线时监控刷新间隔自动放宽到>=10s(引擎自身5s写JSON, 页面无需高频轮询, 省shell)
    //  v21.0.7 改动(UI统一 + 遮挡修复):
    //    X1 修复Dock(z-index:50)盖住弹窗 → z-index:0, 保留悬浮吸顶, 弹窗按DOM序正常置顶
    //    X2 弹窗液态玻璃化(blur+圆角20+阴影, 与主界面同风格); 全部弹窗标题emoji换几何图标
    //    X3 设置页签6个按钮合并为一行(等宽自适应)
    // ============================================================
    ...
    const PLUGIN_VERSION = '21.0.7';
    const _SIG = '@@KANO_TRAFFIC_PLUGIN_ID:5d1f8b@@';
    const _PS = '<!-- [KANO_PLUGIN_START]';
    const _PE = '<!-- [KANO_PLUGIN_END]';
    // v21.0.6(W3): 抽象几何图标(替代emoji, 各设备字体渲染一致且不占emoji字体)
    const ICON = { dockTraffic: '\u25C8', dockHistory: '\u25A3', dockSettings: '\u25C9', up: '\u2191', down: '\u2193', total: '\u03A3', online: '\u25CF', offline: '\u25CB', wifi: '\u25CE', wired: '\u25C9', refresh: '\u21BB', monitor: '\u25B6', monitorOn: '\u23F8', diag: '\u25D0', log: '\u25CC', rank: '\u25CA', archive: '\u25A3', engine: '\u25C8', clear: '\u25EA', uninstall: '\u25EB', rename: '\u270E', del: '\u2715', limit: '\u29D7', time: '\u25F7', runtime: '\u25F4', device: '\u25CA' };
    ...
    const getRefreshInterval = () => {
        ...
        // v21.0.6(V6): 引擎在线时页面刷新放宽到 >=10s(引擎自身5s写JSON, 高频轮询无意义还费shell); engineActive 在后面定义, 运行期调用无TDZ
        return engineActive() ? Math.max(ms, 10000) : ms;
    };
    ...
    // v21.0.5(U1): 引擎在线判定(JSON可用) —— 在线时完全停用 iptables 计数链, 限速链不受影响
...
// B5: 显式转义 & < > " ' （修复属性注入逃逸）
    ...
    const applyLimits = async () => {
        ...
        if (!hasActiveLimits()) {
            ...
            try { await _shUser(`rm -f ${LIMITS_APPLY_SH} 2>/dev/null || true`); } catch (e) {} // v21.0.6(V3)
            return;
        }
        ...
        // v21.0.6(V3): 限速规则持久化为可执行脚本 —— 后台守护在页面关闭期间限速链被清(重启/他组件清表)时可自动重建
        try {
            const sh = '#!/bin/sh\n' + cmds4.concat(cmds6).join('\n') + '\n';
    ...
    const uninstallPlugin = async () => {
        ...
        // 4.7. v21.0.6(V4): 停止后台守护、摘除其自启行、删除守护/计划/触发/限速脚本全部文件
        ...
        await _shUser(`rm -f ${BG_SCRIPT} ${BG_PID} ${BG_SCHED} ${BG_TRIGGER} ${LIMITS_APPLY_SH} 2>/dev/null || true`);
    ...
    const getWifiSignal = async () => {
        ...
        // v21.0.6(V2): iw 之后再加 iwinfo 兜底(部分固件只带 iwinfo), 仍无数据显示 --
        if (Object.keys(st).length === 0) {
            try {
    ...
    const fetchDevs = async () => {
        ...
        if (eng) {
            // v21.0.6(V1): 引擎自带DHCP租约主机名, 引擎在线不再打租约shell(每轮少1次shell)
            const [wifiSig, accessInfo] = await Promise.all([
                getWifiSignal(),
                getAccessDeviceInfo()
            ]);
            const leaseNames = {};
                ...
                html += `<tr class="kano-tr" style="border-bottom:1px solid rgba(255,255,255,0.04);" data-mac="${d.mac}">
                    <td class="kano-td" style="padding:10px 8px;">
                        <div class="kano-hostname" style="font-weight:700;font-size:13px;display:flex;align-items:center;gap:6px;">
                            ${escHtml(d.hostname)}
                            ${d.connType ? `<span style="font-size:9px;opacity:.5;font-weight:400;" title="连接方式(来自接入设备列表)">${d.connType === '有线' ? ICON.wired + '有线' : d.connType === '无线' ? ICON.wifi + '无线' : escHtml(d.connType)}</span>` : ''}
    ...
    const showEngineModal = async () => {
        if (pluginUninstalled) return;
        const { id, el } = createModal({
            name: 'traffic_engine_modal', title: '◈ 统计引擎 (kano_engine)', maxWidth: 'min(480px, 94vw)',
            showConfirm: false, onClose: () => true,
...
const showDeleteConfirm = (mac, ip, ip6) => {
        ...
        const { id, el } = createModal({
            ...
            onConfirm: async () => {
                ...
                if (action === 'clear') {
                    ...
                    if (!engineActive()) await syncRules(); // v21.0.5(U1)
    ...
    const showDiagModal = async () => {
        ...
        // v21.0.6(V5): TUN 代理检测 —— Clash/VPN 分流(0.0.0.0/1+128.0.0.0/1 或 tun 网卡)时 iptables FORWARD 计数链统计不到代理流量
        try {
            ...
            if (tunOn) {
                ...
                auditHtml += `<div style="background:rgba(96,165,250,0.06);border:1px solid rgba(96,165,250,0.15);border-radius:8px;padding:10px;margin-bottom:10px;">
                    ...
                    <div style="font-size:11px;opacity:.75;margin-top:4px;line-height:1.5;">检测到 TUN 代理(0.0.0.0/1 分流或 tun 网卡)${engOk ? '；引擎直读 conntrack，统计不受影响 ✓' : '；<span style="color:#fbbf24;">当前 iptables 计数链可能统计不到代理流量，建议在「◉ 设置 → ◈ 引擎」安装统计引擎</span>'}</div>
                </div>`;
            }
        } catch (e) {}
...
// ============================================================
    ...
    const refresh = async () => {
        ...
        try {
            const engOn = !!(await readEngineJson(true)); // v21.0.5(U1): 引擎在线则完全跳过计数链
            if (!engOn) {
                await initChain();
    ...
    const monitorTick = async () => {
        ...
        try {
            ...
            if (!engineActive() && monitorTickCount < fullSyncEvery && Date.now() - _lastProbe >= 2000) { // v21.0.5(U1): 引擎在线时设备发现由引擎负责, 跳过探针
                _lastProbe = Date.now();
                try {
            ...
            if (monitorTickCount >= fullSyncEvery) {
                monitorTickCount = 0;
                const engOn = !!(await readEngineJson(true)); // v21.0.5(U1): 引擎在线跳过计数链维护
                if (!engOn) {
                    await initChain();
                    if (hasIp6tables && enableIPv6) await initChain6();
                }
    ...
    const clearAllTraffic = async () => {
        ...
        _refreshing = true;
        try {
        const engOn = !!(await readEngineJson(true)); // v21.0.5(U1): 引擎模式只重置基线, 不动计数链
        if (!engOn) {
            await flushChain();
            await initChain();
    ...
    // v21.0.6(W2): 设备趋势 —— 任选一台设备, 按归档记录画纯 CSS 柱状图(无图表库, 零依赖)
    const renderHistoryTrend = (box) => {
        if (!box) return;
        ...
        if (macSet.size === 0) { box.innerHTML = '<div style="text-align:center;padding:30px;opacity:.5;">暂无归档记录</div>'; return; }
        const cur = (box.dataset.mac && macSet.has(box.dataset.mac)) ? box.dataset.mac : [...macSet.keys()][0];
        box.dataset.mac = cur;
        const rows = [];
        let maxV = 1;
        for (const sn of snapshots) {
            ...
            const v = d.total || 0;
            if (v > maxV) maxV = v;
            rows.push({ label: sn.label, v });
        }
        let html = '<select id="kano_trend_mac" style="width:100%;box-sizing:border-box;font-size:11px;padding:4px 6px;margin-bottom:10px;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:4px;color:inherit;">' +
        ...
        if (rows.length === 0) {
            ...
            for (const r of show) {
                ...
                html += `<div title="${escHtml(r.label)} \u00B7 ${formatBytes(r.v)}" style="flex:1;min-width:8px;height:${h}px;background:linear-gradient(180deg,var(--dark-btn-color-active),#4ade80);border-radius:3px 3px 0 0;"></div>`;
            }
            html += `</div><div style="font-size:10px;opacity:.45;margin-top:6px;">最近 ${show.length} 次归档 \u00B7 悬停柱子查看数值 \u00B7 峰值 ${formatBytes(maxV)}</div>`;
        }
        box.innerHTML = html;
        box.querySelector('#kano_trend_mac')?.addEventListener('change', (e) => { box.dataset.mac = e.target.value; renderHistoryTrend(box); });
    };
    ...
    const showHistoryModal = () => {
        ...
        const { id, el } = createModal({
            ...
            content: `<div style="display:flex;gap:6px;margin-bottom:8px;">
                    <button class="kano-hist-tab" data-htab="all" style="flex:1;font-size:11px;padding:5px;border-radius:8px;border:1px solid rgba(255,255,255,0.1);background:rgba(255,255,255,0.08);color:inherit;cursor:pointer;">▣ 全部归档</button>
                    <button class="kano-hist-tab" data-htab="trend" style="flex:1;font-size:11px;padding:5px;border-radius:8px;border:1px solid rgba(255,255,255,0.1);background:transparent;color:inherit;cursor:pointer;opacity:.5;">◊ 设备趋势</button>
                </div>
                <div id="kano_history_content" style="font-size:12px;line-height:1.5;max-height:420px;overflow:auto;"></div>
                <div id="kano_history_trend" style="display:none;font-size:12px;line-height:1.5;max-height:420px;overflow:auto;"></div>`
        });
        showModal(id);
        renderHistoryContent(el.querySelector('#kano_history_content'));
        // v21.0.6(W2): 全部归档 / 设备趋势 标签切换
        el.querySelectorAll('.kano-hist-tab').forEach(b => b.addEventListener('click', () => {
            const trend = b.dataset.htab === 'trend';
            el.querySelectorAll('.kano-hist-tab').forEach(x => { x.style.background = x === b ? 'rgba(255,255,255,0.08)' : 'transparent'; x.style.opacity = x === b ? '1' : '.5'; });
            el.querySelector('#kano_history_content').style.display = trend ? 'none' : 'block';
            el.querySelector('#kano_history_trend').style.display = trend ? 'block' : 'none';
            if (trend) renderHistoryTrend(el.querySelector('#kano_history_trend'));
        }));
    };
    ...
    // v21.0.6(V4): 后台守护 —— 页面全关时设备侧 60s 循环兜底: 到点写归档触发文件 + 限速链被清时用持久化脚本重建
    const installBgDaemon = async () => {
        ...
        try {
            ...
            const script = '#!/bin/sh\n' +
                ...
                '  if [ "$sched" -gt 0 ] && [ "$now" -ge "$sched" ]; then echo "$now" > ' + BG_TRIGGER + '; echo 0 > ' + BG_SCHED + '; fi\n' +
    ...
    // 按归档设置计算下次到点 epoch 写入(0=无计划); 守护脚本到点写触发文件
    ...
    // 触发文件存在且本端为心跳主端 → 消费并走自动归档判定(页面关闭期间守护脚本发起的到点归档)
    const checkBgTrigger = async () => {
        if (pluginUninstalled) return;
        try {
            const r = await _shUser(`[ -f ${BG_TRIGGER} ] && echo TRIG || echo NONE`);
            if (!_sh(r).includes('TRIG')) return;
            if (!(await isHeartbeatLeader())) return;
            await _shUser(`rm -f ${BG_TRIGGER} 2>/dev/null || true`);
            _log('ACTION', '后台守护到点 → 心跳主端执行自动归档检查');
    ...
    const startAutoArchive = () => {
        ...
        writeBgSched();  // v21.0.6(V4)
        checkBgTrigger();
        // v21.0.6(V4): 5分钟→60s —— 及时消费后台守护的到点触发(页面开着时), 判定本身仍只在到点时打shell
    ...
    const showSettingsModal = () => {
        ...
        const { id, el } = createModal({
            name: 'traffic_settings_modal', title: '◉ 设置', maxWidth: 'min(400px, 94vw)',
            ...
            onConfirm: async () => {
                ...
                saveSettings();
                writeBgSched(); // v21.0.6(V4): 归档计划变更同步给后台守护
                ...
                if (enableIPv6 && !prevEnableIPv6) {
                    // 开启：同步 init v6 链与规则
                    const engOn6 = engineActive(); // v21.0.5(U1): 引擎在线跳过计数链
    ...
    // v21.0.6(W4): 手动手机视图开关已删除 —— 容器级 kano-compact 响应式自动适配，无需手动切换
    ...
    mmContainer.insertAdjacentHTML("afterend", `
        <style>
            ...
            /* v21.0.7(X2): 弹窗液态玻璃化(宿主弹窗元素id=createModal的name, 与主界面同风格) */
            #traffic_history_modal, #traffic_rank_modal, #traffic_log_modal, #traffic_diag_modal,
            #traffic_engine_modal, #traffic_settings_modal, #traffic_limit_modal, #traffic_archive_modal,
            #traffic_delete_modal, #traffic_rename_modal, #traffic_limit_diag_modal, #traffic_export_modal {
                background:rgba(18,20,28,0.72) !important; backdrop-filter:blur(24px) saturate(180%); -webkit-backdrop-filter:blur(24px) saturate(180%);
                border:1px solid rgba(255,255,255,0.08) !important; border-radius:20px !important; box-shadow:0 8px 32px rgba(0,0,0,0.3) !important;
            }
            /* v21.0.7(X3): 设置页签按钮单行等宽 */
            #IFRAME_KANO_TRAFFIC .kano-settings-row { flex-wrap:nowrap !important; }
            #IFRAME_KANO_TRAFFIC .kano-settings-row button { flex:1; min-width:0; font-size:10px !important; padding:4px 2px !important; }
            /* v21.0.6(W4): 总流量列小字行(↑↓明细+速率), 仅 compact 下显示(上/下载列被隐藏时兜底) */
            ...
            /* v21.0.6(W4): compact 下隐藏上/下载两列(小字行兜底显示明细) */
            ...
            <div class="collapse" id="collapse_traffic" data-name="close" style="height:0px;overflow:hidden;">
                <div class="collapse_box">
                    ...
                    <div class="kano-tabpane" data-pane="history">
                        <div class="kano-btn-row" style="margin-bottom:10px;">
                            <button id="kano_traffic_archive_btn" style="background:rgba(167,139,250,0.1);color:#a78bfa;border-color:rgba(167,139,250,0.2);font-size:12px;padding:4px 10px;">${ICON.archive} 归档并重计</button>
                            <button id="kano_traffic_history_btn" style="background:rgba(96,165,250,0.1);color:#60a5fa;border-color:rgba(96,165,250,0.2);font-size:12px;padding:4px 10px;">${ICON.dockHistory} 查看历史</button>
                            <button id="kano_traffic_rank_btn" style="background:rgba(74,222,128,0.1);color:#4ade80;border-color:rgba(74,222,128,0.2);font-size:12px;padding:4px 10px;">${ICON.rank} 流量排行</button>
                        </div>
                        <div style="font-size:11px;opacity:.5;line-height:1.6;padding:4px 2px;">▣ 日报/月报归档、历史记录与流量排行在这里管理</div>
                    </div>
                    <div class="kano-tabpane" data-pane="settings">
                        <div class="kano-btn-row kano-settings-row" style="margin-bottom:10px;">
                            <button id="kano_traffic_engine_btn" style="background:rgba(45,212,191,0.1);color:#2dd4bf;border-color:rgba(45,212,191,0.2);font-size:12px;padding:4px 10px;">${ICON.engine} 引擎</button>
                            <button id="kano_traffic_settings_btn" style="background:rgba(148,163,184,0.1);color:#94a3b8;border-color:rgba(148,163,184,0.2);font-size:12px;padding:4px 10px;">${ICON.dockSettings} 设置</button>
                            <button id="kano_traffic_log_btn" style="background:rgba(56,189,248,0.1);color:#38bdf8;border-color:rgba(56,189,248,0.2);font-size:12px;padding:4px 10px;">${ICON.log} 日志</button>
                            <button id="kano_traffic_diag_btn" style="font-size:12px;padding:4px 10px;">${ICON.diag} 诊断</button>
                            <button id="kano_traffic_cleartraffic_btn" style="background:rgba(251,146,60,0.1);color:#fb923c;border-color:rgba(251,146,60,0.2);font-size:12px;padding:4px 10px;">${ICON.clear} 清除</button>
                            <button id="kano_traffic_uninstall_btn" style="background:rgba(120,120,120,0.1);color:#888;border-color:rgba(255,255,255,0.1);font-size:12px;padding:4px 10px;">${ICON.uninstall} 卸载</button>
                        </div>
                    </div>
                    <div style="margin-top:10px;font-size:10px;opacity:.45;text-align:center;">
                        conntrack引擎统计(在线时自动停用iptables计数链) | 数据保存在设备内 | ${ICON.rename}改名 ${ICON.del}删除 ${ICON.archive}归档 ${ICON.rank}排行 | v${PLUGIN_VERSION}
                    </div>
                </div>
            </div>
        </div>
    ...
    // v21.0.6(W1): Dock 页签切换 + 状态栏时钟(系统时间/运行时间, 1s)
    ...
    const _sbTick = () => {
        ...
        if (rEl) { const mm = Math.floor((Date.now() - pluginStartTime) / 60000); rEl.textContent = ICON.runtime + ' ' + (mm >= 60 ? Math.floor(mm / 60) + 'h ' + (mm % 60) + 'm' : mm + 'm'); }
    };
    _sbTick();
    ...
    startAutoArchive();
    installBgDaemon(); // v21.0.6(V4): 后台守护(到点归档触发 + 限速链守护), 幂等
