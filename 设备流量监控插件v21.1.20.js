...
(async () => {
    ...
    const renderOverview = () => {
        ...
        el.innerHTML = `<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(72px,1fr));gap:8px;text-align:center;">
    ...
    const mockEngineJson = () => {
        ...
        return { updatedAt: new Date().toLocaleString(), ts: Math.floor(Date.now() / 1000), engineRev: 'mock', wan: 'mock0', conntrack: 1, acct: 1,
            summary: { sysDeltaBytes: tot, sysDeltaTxBytes: 0, sysDeltaRxBytes: 0, iptTotalBytes: tot, iptTotalV4Bytes: Math.floor(tot * 0.6), iptTotalV6Bytes: Math.floor(tot * 0.4), deviceTotalBytes: tot, deviceCount: 3, zeroStreak: 0 },
            devices };
    };
    ...
    const renderEngineInline = async () => {
        ...
        container.querySelector('#kano_eng_fixture_inline')?.addEventListener('click', async (ev) => {
            ev.target.disabled = true; ev.target.textContent = '自检中(约10s)...';
            let out = '';
            try {
                const r = await _shUser(`D=/data/local/tmp/kano_fixtures; mkdir -p $D; C=curl; [ -x /data/data/com.minikano.f50_sms/files/curl ] && C=/data/data/com.minikano.f50_sms/files/curl; ok=0; for u in 'https://ghproxy.net/https://raw.githubusercontent.com/468133/kano-engine-assets/main/fixtures/run_fixtures.sh' 'https://ghfast.top/https://raw.githubusercontent.com/468133/kano-engine-assets/main/fixtures/run_fixtures.sh' 'https://cdn.jsdmirror.com/gh/468133/kano-engine-assets@main/fixtures/run_fixtures.sh' 'https://jsd.onmicrosoft.cn/gh/468133/kano-engine-assets@main/fixtures/run_fixtures.sh' 'https://cdn.jsdelivr.net/gh/468133/kano-engine-assets@main/fixtures/run_fixtures.sh'; do $C -fsSL --connect-timeout 8 --max-time 25 "$u" -o $D/run.sh && [ -s $D/run.sh ] && ok=1 && break; done; if [ $ok = 1 ]; then sh $D/run.sh 2>&1; else echo 'FIXTURE_DL_FAIL 假数据脚本下载失败(所有源均不可达)'; fi`);
                out = _sh(r).trim() || '(无输出)';
                _log('ENGINE', '假数据自检完成');
    ...
    mmContainer.insertAdjacentHTML("afterend", `
        <style>
            ...
            /* 小UI(田字): 左设备信息 | 田字(上/下载各占半行、总量跨底行) | 最右限速上删除下竖排窄列; v21.1.20 分割线去行距不断开, 左区加宽, 多余高度给底行(auto 1fr)消除顶部空旷 */
            #IFRAME_KANO_TRAFFIC.kano-tiny .kano-tbl tr.kano-tr { display:grid; grid-template-columns:minmax(0,1.5fr) minmax(0,0.95fr) minmax(0,0.95fr) auto; grid-template-rows:auto 1fr; gap:0 8px; padding:6px 4px; align-items:center; }
            ...
            #IFRAME_KANO_TRAFFIC.kano-tiny .kano-tbl tr.kano-tr td:nth-child(1) { grid-column:1; grid-row:1 / 3; padding-right:2px !important; } /* 2+gap8=10, 与分割线右侧 padding-left:10 等距 */
            #IFRAME_KANO_TRAFFIC.kano-tiny .kano-tbl tr.kano-tr td:nth-child(4),
            #IFRAME_KANO_TRAFFIC.kano-tiny .kano-tbl tr.kano-tr td:nth-child(6) { border-left:1px solid rgba(255,255,255,0.08); padding-left:10px !important; }
            #IFRAME_KANO_TRAFFIC.kano-tiny .kano-tbl tr.kano-tr td:nth-child(4) { grid-column:2; grid-row:1; }
            #IFRAME_KANO_TRAFFIC.kano-tiny .kano-tbl tr.kano-tr td:nth-child(5) { grid-column:3; grid-row:1; text-align:right !important; }
            /* 上/下载速度一行显示不堆积, 顶部对齐消除上边距 */
            #IFRAME_KANO_TRAFFIC.kano-tiny .kano-tbl tr.kano-tr td:nth-child(4),
            #IFRAME_KANO_TRAFFIC.kano-tiny .kano-tbl tr.kano-tr td:nth-child(5) { white-space:nowrap !important; align-self:start; padding-top:3px !important; }
            #IFRAME_KANO_TRAFFIC.kano-tiny .kano-tbl tr.kano-tr td:nth-child(4) > div,
            #IFRAME_KANO_TRAFFIC.kano-tiny .kano-tbl tr.kano-tr td:nth-child(5) > div { display:inline; }
            #IFRAME_KANO_TRAFFIC.kano-tiny .kano-tbl tr.kano-tr td:nth-child(6) { grid-column:2 / 4; grid-row:2; }
            #IFRAME_KANO_TRAFFIC.kano-tiny .kano-tbl tr.kano-tr td:nth-child(7) { grid-column:4; grid-row:1 / 3; text-align:center !important; display:flex !important; flex-direction:column; align-items:center; justify-content:center; gap:5px; padding:2px 0 !important; } /* 限速在上、删除在下，竖排窄列 */
            #IFRAME_KANO_TRAFFIC.kano-tiny .kano-tbl .kano-mac,
            #IFRAME_KANO_TRAFFIC.kano-tiny .kano-tbl .kano-signal { display:none !important; }
    ...
    const pickKao = () => {
        ...
        if (!el || !root) return;
        // v21.1.20: 以 applyResponsive 计算的档位为单一事实源, 防类名抖动导致小UI抽到中/大档文案
        const g = _kaoGroup || (root.classList.contains('kano-tiny') ? 'tiny' : (root.classList.contains('kano-compact') ? 'mid' : 'big'));
