...
(async () => {
    // ============================================================
    //  设备流量监控插件 v21.1.54 (清除流量联动清零引擎 WAN 系统增量)
    ...
    const clearAllTraffic = async () => {
        ...
        try {
            ...
            if (!engOn) {
                ...
                await resetEngineSys(false);
            }
            const now = Date.now();
            resetHistoryEntries(now, true);
            _localResetAt = Date.now(); // 合并守卫: 防止磁盘旧值复活
            _engineJsonCache = { t: 0, data: null }; // v21.1.54: 使 renderOverview/下轮 tick 读取清零后的引擎 JSON
            await flushHistory(true);
            renderList();
            renderOverview();
