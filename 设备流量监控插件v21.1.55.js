...
(async () => {
    // ============================================================
    //  设备流量监控插件 v21.1.55 (未归属地址反查实际设备 / 清除流量后归因即时刷新)
    ...
    const clearAllTraffic = async () => {
        ...
        try {
            ...
            if (!engOn) {
                ...
                if (sent) await waitEngineSysZeroed((engPre && engPre.summary && engPre.summary.sysDeltaBytes) || 0);
            }
            const now = Date.now();
