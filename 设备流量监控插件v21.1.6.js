...
(async () => {
    ...
    let _tinyView = false;
    const _hostFmtBytes = formatBytes;
    const formatBytes = (b) => {
    ...
    const exportDiagPack = async () => {
        ...
        L.push('======== ⑤ 引擎进程与口径核对 ========');
        try {
            const r5 = await runShellWithRoot(`echo "-- 引擎进程(应只有1个) --"; ps -ef 2>/dev/null | grep kano_engine | grep -v grep || ps 2>/dev/null | grep kano_engine | grep -v grep || echo '(ps不可用)'; echo "-- 引擎JSON summary --"; head -c 500 /data/data/com.minikano.f50_sms/kano_engine.json 2>/dev/null || echo '(无JSON)'; echo; echo "-- 路由表 --"; ip route 2>/dev/null || echo '(ip命令不可用)'`);
        ...
        container.innerHTML = `
            ...
            <div style="margin-top:12px;padding:8px;background:rgba(255,255,255,0.03);border-radius:6px;font-size:10px;text-align:center;">
                ...
                <div style="margin-top:8px;">
                    <a href="https://t.me/UFI_TOOLS_TG_GUMU51" target="_blank" style="color:#38bdf8;text-decoration:none;font-size:11px;font-weight:600;">✈️ TG问题反馈: @UFI-TOOLS-TG@GUMU51</a>
                </div>
            </div>`;
    ...
    mmContainer.insertAdjacentHTML("afterend", `
        <style>
            ...
            #IFRAME_KANO_TRAFFIC.kano-tiny .kano-tbl tr.kano-tr { display:grid; grid-template-columns:1fr 1fr; gap:1px 8px; padding:6px 4px; }
            ...
            #IFRAME_KANO_TRAFFIC.kano-tiny .kano-tbl tr.kano-tr td:nth-child(1) { grid-column:1; grid-row:1; }
            #IFRAME_KANO_TRAFFIC.kano-tiny .kano-tbl tr.kano-tr td:nth-child(6) { grid-column:2; grid-row:1; text-align:right !important; }
            #IFRAME_KANO_TRAFFIC.kano-tiny .kano-tbl tr.kano-tr td:nth-child(4) { grid-column:1; grid-row:2; }
            #IFRAME_KANO_TRAFFIC.kano-tiny .kano-tbl tr.kano-tr td:nth-child(5) { grid-column:2; grid-row:2; text-align:right !important; }
            #IFRAME_KANO_TRAFFIC.kano-tiny .kano-tbl tr.kano-tr td:nth-child(7) { grid-column:1 / 3; grid-row:3; text-align:right !important; }
            #IFRAME_KANO_TRAFFIC.kano-tiny .kano-tbl .kano-mac,
            #IFRAME_KANO_TRAFFIC.kano-tiny .kano-tbl .kano-signal { display:none !important; }
            #IFRAME_KANO_TRAFFIC.kano-tiny .kano-tbl tr.kano-tr td div[style*="height:3px"] { width:56px !important; margin-left:auto; }
            ...
            <div class="collapse" id="collapse_traffic" data-name="close" style="height:0px;overflow:hidden;">
                <div class="collapse_box">
                    ...
                    <div style="margin-top:10px;font-size:10px;opacity:.45;text-align:center;">
                        conntrack引擎统计(在线时自动停用iptables计数链) | 数据保存在设备内 | ${ICON.rename}改名 ${ICON.del}删除 ${ICON.archive}归档 ${ICON.rank}排行 | v${PLUGIN_VERSION}<br>
                        <a href="https://t.me/UFI_TOOLS_TG_GUMU51" target="_blank" style="color:#38bdf8;text-decoration:none;">✈️ 问题反馈 TG: @UFI-TOOLS-TG@GUMU51</a>
                    </div>
                </div>
            </div>
        </div>
